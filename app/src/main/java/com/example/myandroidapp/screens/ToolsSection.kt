...
fun BlockTab(nav: NavController, viewModel: SharedAppViewModel, title: String) {
    ...
    Column(Modifier.fillMaxSize().padding(16.dp)) {
        ...
        VisualBlockBuilder(filters) {
            viewModel.recordFeature("add_filter", mapOf("tab" to title, "count" to filters.size.toString()))
            filters.add(AutomationFilter("", "", ""))
        }
        Spacer(Modifier.height(16.dp))
        Button(onClick = {
            ...
            viewModel.recordFeature(
                "save_automation",
                mapOf("tab" to title, "filters" to filters.size.toString())
            )
