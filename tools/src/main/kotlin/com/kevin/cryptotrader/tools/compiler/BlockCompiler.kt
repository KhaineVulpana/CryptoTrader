...
object BlockCompiler {
  ...
  fun compile(jsonStr: String): String {
    ...
    val edges = doc.graph.edges
    val upstreamOf = edges.groupBy({ it[1] }, { it[0] })
    val crossesNodes = doc.graph.nodes.filter { it.type.startsWith("crosses") }
    crossesNodes.forEachIndexed { idx, node ->
      // Try to resolve up to two levels upstream to find indicator outputs
      val u1 = upstreamOf[node.id].orEmpty()
      val u2 = u1.flatMap { upstreamOf[it].orEmpty() }
      val candidates = (u1 + u2)
      val seriesNames = candidates.mapNotNull { idToSeriesName[it] }.distinct()
